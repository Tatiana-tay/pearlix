...
| 6 | Leave exceptions | Ready for ChatGPT review | Temporary leave/block model. | AvailabilityException model; startAt/endAt UTC; reason/status; create/update/cancel; does not mutate WorkingShift; affected appointment logic can be minimal until appointments exist, then completed in Phase 8/9. | Appointment scheduling beyond minimal future hook; mutating WorkingShift for leave. | Invalid interval rejected; cancel leave; leave does not edit shifts; permission tests. | makemigrations; migrate; test; check. | Temporary leave exists separately from recurring schedule. | Pending review |
...
```text
Current phase: 6 - Leave exceptions
Current status: Ready for ChatGPT review
Next required action: ChatGPT must review Phase 6 implementation report.
```
