...
| 9 | Visit lifecycle | Ready for ChatGPT review | Doctor clinical visit workflow. | Visit model; Doctor starts only own Checked-in appointment; appointment becomes In Visit; one active visit per doctor; notes fields; complete visit; appointment becomes Completed; version/updatedAt. | Invoice payment; attachments; AI inference. | Doctor cannot start Arrived appointment; Doctor cannot start another doctor's appointment; Doctor cannot have two active visits; Admin/Staff cannot clinically complete visit; complete visit updates appointment. | makemigrations; migrate; test; check. | Visit lifecycle is atomic and permission-safe. | Pending review |
...
```text
Current phase: 9 - Visit lifecycle
Current status: Ready for ChatGPT review
Next required action: ChatGPT must review Phase 9 implementation report.
```
