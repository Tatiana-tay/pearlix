...
| 2 | Clinic settings | Ready for ChatGPT review | Clinic-wide timezone and capacity settings. | ClinicSettings model; singleton behavior; `clinicTimezone`; `maxSimultaneousAppointments`; validation for IANA timezone; API get/update. | Appointment scheduling; dashboard reports/charts. | Valid timezone accepted; invalid timezone rejected; capacity validation; permission tests. | makemigrations; migrate; test; check. | Scheduling authority settings exist before appointments. | Pending review |
...
```text
Current phase: 2 - Clinic settings
Current status: Ready for ChatGPT review
Next required action: ChatGPT must review Phase 2 implementation report.
```
